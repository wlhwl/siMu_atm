...
def drawArrayAs1DHist(arr, nbins=20, logx=False, logy=False, xmin='', xmax='',
                       xlabel='', ylabel='', label='', title='', filename=None,
                       draw_quantile=[], median=False, mean=False, peak=False,
                       density=False, weight=None, show_data_size=False,
                       ax=None, fig=None, figsize=(5, 4), dpi=400, color=None):
    arr = np.array(arr)
    ...
    if xmax=='':
        xmax = max(arr)
    bins = np.logspace(np.log10(xmin), np.log10(xmax), nbins) if logx else np.linspace(xmin, xmax, nbins)

    counts, bin_edges = np.histogram(arr, bins, density=density, weights=weight)
    plt.hist(bin_edges[:-1], bin_edges, weights=counts, histtype='step', color=color,label=label)
    
    if peak:
        peak_bin_center = (bin_edges[np.argmax(counts)] + bin_edges[np.argmax(counts) + 1]) / 2
        ax.axvline(x=peak_bin_center, linestyle='-.', label=f'peak bin={peak_bin_center:.2f}', color="xkcd:purple")

    if mean:
        ax.axvline(x=np.mean(arr), linestyle='-.', label=f'mean={np.mean(arr):.2f}', color="xkcd:red")
    if median:
        ax.axvline(x=np.median(arr), linestyle='-.', label=f'median={np.median(arr):.2f}', color="xkcd:orange")
   
    color_ = ["xkcd:sky blue", "xkcd:salmon", "xkcd:lime", "xkcd:violet", "xkcd:yellow"]
    for i, quantile in enumerate(draw_quantile):
        color__ = color_[i % len(color_)]
        ax.axvline(x=np.quantile(arr, quantile), linestyle='--', label=f'{quantile*100}%', color=color__)
...
def drawArraysAs2DHist(arr1, arr2, nbins=30, logx=False, logy=False, logz=False,
                       xmin='',xmax='',ymin='',ymax='',
    ...
    xedges = np.logspace(np.log10(xmin), np.log10(xmax), nbins) if logx else np.linspace(xmin, xmax, nbins)
    yedges = np.logspace(np.log10(ymin), np.log10(ymax), nbins) if logy else np.linspace(ymin, ymax, nbins)
    hist, _, _ = np.histogram2d(arr1, arr2, bins=[xedges, yedges])
